...
while i < len(buffer):
    ...
    if i == len(buffer):
        segmnt = _SEGMENT.SEGMENT(_pTYPES.END, seq, 0, 0, 0,
                                  sub_buffer)  # pkt_type, pkt_seq, pkt_ack, pkt_ratio, opt, pkt_data
    else:
        segmnt = _SEGMENT.SEGMENT(_pTYPES.CHUNK, seq, 0, 0, 0,
                                  sub_buffer)  # pkt_type, pkt_seq, pkt_ack, pkt_ratio, opt, pkt_data
    seq += 1
    packet = segmnt.encap()
